...
Lessons baked in (from re-reading study/baas/module/{mini_story,
...
Key fixes vs v4:
  ...
  * v4 fallback chapter anchors were at x=0.78 (mid-row body); BAAS
  ...
  * Cutscene confirm-dialog signal is 概要 (BAAS plot_skip-plot-
    notice header), not "是否略過".

Architecture remains BAAS-style flat reactive rule list (single
...
# Card click centres (book layout, 2 cols x 3 rows).  BAAS pixel
...
# 'New' yellow badge top-left anchors per card.  BAAS check_6_region_
...
# Page-next arrow.  BAAS clicks (1255, 357).  We can't probe its
# active-colour RGB; instead we detect grid-fingerprint stalls + cap
# at 5 pages per category (BAAS short / side ship 2-3 pages).
...
# 入場 button click anchors.  BAAS one_detect probes (1073, 251) and
...
# Volume-cleared signal — BAAS image: mini_story_episode-cleared-
# feature.png == "已完成所有章節".
...
# 進入章節 button at bottom-centre of the modal.  BAAS clicks
# (650, 511) -> (0.508, 0.710).
...
# BAAS plot_menu (1202, 37) / plot_skip-plot-button (1208, 116) /
...
class StoryMiningSkill(BaseSkill):
    """Auto-play unplayed chapters in 短篇劇情 / 支線劇情 — BAAS-
    ...
    def _handle_reward_popup(
        ...
        # Post-cutscene reward splash.  BAAS uses RGB at (640, 100);
    ...
    def _on_chapter_info(self, screen: ScreenState) -> Dict[str, Any]:
        ...
        # Fallback to BAAS's fixed coord (650, 511).
