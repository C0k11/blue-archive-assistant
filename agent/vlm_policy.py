...
class VlmPolicyAgent:
    ...
    def _decide(self, *, screenshot_path: str) -> Dict[str, Any]:
        with Image.open(screenshot_path) as im:
            w, h = im.size
    ...
    def _execute(self, action: Dict[str, Any], *, screenshot_path: str) -> None:
        a = str(action.get("action", "")).lower().strip()
        if a == "wait":
        ...
        if a == "click":
            ...
            if isinstance(target, (list, tuple)) and len(target) == 2:
                self._device.click_client(int(target[0]), int(target[1]))
                return
        ...
        if a == "swipe":
            ...
            self._device.swipe_client(int(p1[0]), int(p1[1]), int(p2[0]), int(p2[1]), duration_ms=d)
            return
    ...
    def _run_loop(self) -> None:
        ...
        while not self._stop.is_set():
            ...
            try:
                ...
                act = self._decide(screenshot_path=shot_path)
                act_before = act
                ...
                act = self._sanitize_action(act)
                self._execute(act, screenshot_path=shot_path)
                with self._lock:
